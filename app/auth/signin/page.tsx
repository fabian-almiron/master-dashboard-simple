...
export default function SignIn() {
  ...
  const handleSignIn = async (providerId: string) => {
    setIsLoading(true)
    await signIn(providerId, { callbackUrl: '/master' })
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-950 via-gray-900 to-black relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md mx-4">
            ...
            {providers &&
              Object.values(providers).map((provider: any) => (
                <Button
                  ...
                  className={`
                    w-full h-12 relative overflow-hidden group transition-all duration-300 transform hover:scale-105
                    ${provider.name === 'Google' 
                      ? 'bg-white hover:bg-gray-50 text-gray-900 border-0 shadow-lg hover:shadow-xl' 
                      : 'bg-gray-800 hover:bg-gray-700 text-white border border-gray-700 hover:border-gray-600 shadow-lg hover:shadow-xl'
                    }
                    ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}
                    ${hoveredProvider === provider.name ? 'ring-2 ring-blue-400/50' : ''}
                  `}
                >
        ...
        <div className="text-center mt-8 text-gray-500 text-sm">
          <p>© 2024 Altira. All rights reserved.</p>
        </div>
      </div>
